 ...
 */
@Component
public class PrepareNiftyTotal750WeeklyReport {
	...
	private static Logger LOG = LoggerFactory.getLogger(PrepareNiftyTotal750WeeklyReport.class);
	...
	public void createDailyReport() {
		...
		List<NiftyTotal750Model> niftySmallCap50ModelList = niftyTotal750ModelService.findAll();
		List<Integer> listOfEquitySymbolInNiftyTotal750 = niftySmallCap50ModelList.stream().map(model -> model.getEquitySymbolId()).collect(Collectors.toList());
		
		List<PriceModel> listOfPriceModelInNiftyTotal750 = list.stream().filter(model -> listOfEquitySymbolInNiftyTotal750.contains(model.getEquitySymbolId())).collect(Collectors.toList());
		// Get date
		...
		createFivePercentGainerReport(listOfPriceModelInNiftyTotal750, strDate, mappingStrategy);

		// prepare a list of stocks fall more than 5%
		createFivePercentLosserReport(listOfPriceModelInNiftyTotal750, strDate, mappingStrategy);
	}
	...
	private void createFivePercentGainerReport(List<PriceModel> list, String strDate, CustomMappingStrategy<DailyReportModel> mappingStrategy) {
		...
		File file = new File(DAILY_LOC_PREFIX + "NiftyTotal750FivePercentGainer" + strDate + ".csv");
		if (!file.exists()) {
			try {
				file.getParentFile().mkdir();
				file.createNewFile();
			} catch (IOException e) {
				LOG.error("Error in create file for NiftyTotal750FivePercentGainer : " + e.getMessage());
			}
		}
		try (Writer writer = new PrintWriter(file)) {
			...
			LOG.error("Exception occured in daily report processing for NiftyTotal750FivePercentGainer : "
					+ e.getMessage());
		}
	...
	private void createFivePercentLosserReport(List<PriceModel> list, String strDate, CustomMappingStrategy<DailyReportModel> mappingStrategy) {
		...
		// prepare file for greater than five percent
		File file = new File(DAILY_LOC_PREFIX + "NiftyTotal750FivePercentLooser" + strDate + ".csv");
		if (!file.exists()) {
			try {
				file.getParentFile().mkdir();
				file.createNewFile();
			} catch (IOException e) {
				LOG.error("Error in create file for NiftyTotal750FivePercentLooser : " + e.getMessage());
			}
		}
		try (Writer writer = new PrintWriter(file)) {
			StatefulBeanToCsv<DailyReportModel> beanToCsv = new StatefulBeanToCsvBuilder<DailyReportModel>(writer)
			...
			LOG.error("Exception occured in daily report processing for NiftyTotal750FivePercentLooser : "
					+ e.getMessage());
		}
