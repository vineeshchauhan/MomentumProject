 ...
 */
@Component
public class PrepareWeeklyReport {
	...
	/**
	 * The method is responsible for creating 2 reports,
	 *  1- stocks which rises more than 5 percent. 
	 *  2- stocks which falls more than 5 percent.
	 */
	public void createWeeklyReport() {

		// fetch today's(Friday) equity data
		List<PriceModel> listCW = priceModelService.findByDate(LocalDate.now().minus(Period.ofDays(1)));

		// fetch today's(Friday) equity data
		List<PriceModel> listPW = priceModelService.findByDate(LocalDate.now().minus(Period.ofDays(8)));
		...
		createFivePercentGainerReport(listPW, listCW, strDate, mappingStrategy);

		// prepare a list of stocks fall more than 5%
		createFivePercentLosserReport(listPW, listCW, strDate, mappingStrategy);
	}
	...
	private void createFivePercentGainerReport(List<PriceModel> listPW, List<PriceModel> listCW, String strDate,
		...
		File file = new File(WEEKLY_LOC_PREFIX + "FivePercentGainer" + strDate + ".csv");
		if (!file.exists()) {
			try {
				file.getParentFile().mkdir();
				file.createNewFile();
			} catch (IOException e) {
				LOG.error("Error in create file for FivePercentGainer : " + e.getMessage());
			}
		}
		...
		try (Writer writer = new PrintWriter(file)) {
			...
			LOG.error("Exception occured in daily report processing for five percent gainer, fivepergainer : "
					+ e.getMessage());
		}
	...
	private void createFivePercentLosserReport(List<PriceModel> listPW, List<PriceModel> listCW, String strDate,
		...
		// prepare file for greater than five percent
		File file = new File(WEEKLY_LOC_PREFIX + "FivePercentLooser" + strDate + ".csv");
		if (!file.exists()) {
			try {
				file.getParentFile().mkdir();
				file.createNewFile();
			} catch (IOException e) {
				LOG.error("Error in create file for fivePerLooser : " + e.getMessage());
			}
		}
		...
		try (Writer writer = new PrintWriter(file)) {
			...
			LOG.error("Exception occured in daily report processing for fivePerLooser, fivePerLooser : "
					+ e.getMessage());
		}
