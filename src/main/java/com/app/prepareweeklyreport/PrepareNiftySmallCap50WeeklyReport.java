 ...
 */
@Component
public class PrepareNiftySmallCap50WeeklyReport {
	...
	private static Logger LOG = LoggerFactory.getLogger(PrepareNiftySmallCap50WeeklyReport.class);
	...
	public void createDailyReport() {
		...
		List<NiftySmallCap50Model> niftySmallCap50ModelList = niftySmallCap50ModelService.findAll();
		List<Integer> listOfEquitySymbolInNiftySmallCap50 = niftySmallCap50ModelList.stream().map(model -> model.getEquitySymbolId()).collect(Collectors.toList());
		
		List<PriceModel> listOfPriceModelInNiftySmallCap50 = list.stream().filter(model -> listOfEquitySymbolInNiftySmallCap50.contains(model.getEquitySymbolId())).collect(Collectors.toList());
		// Get date
		...
		createFivePercentGainerReport(listOfPriceModelInNiftySmallCap50, strDate, mappingStrategy);

		// prepare a list of stocks fall more than 5%
		createFivePercentLosserReport(listOfPriceModelInNiftySmallCap50, strDate, mappingStrategy);
	}
	...
	private void createFivePercentGainerReport(List<PriceModel> list, String strDate, CustomMappingStrategy<DailyReportModel> mappingStrategy) {
		...
		File file = new File(DAILY_LOC_PREFIX + "NiftySmallCap50FivePercentGainer" + strDate + ".csv");
		if (!file.exists()) {
			try {
				file.getParentFile().mkdir();
				file.createNewFile();
			} catch (IOException e) {
				LOG.error("Error in create file for NiftySmallCap50FivePercentGainer : " + e.getMessage());
			}
		}
		try (Writer writer = new PrintWriter(file)) {
			...
			LOG.error("Exception occured in daily report processing for NiftySmallCap50FivePercentGainer : "
					+ e.getMessage());
		}
	...
	private void createFivePercentLosserReport(List<PriceModel> list, String strDate, CustomMappingStrategy<DailyReportModel> mappingStrategy) {
		...
		// prepare file for greater than five percent
		File file = new File(DAILY_LOC_PREFIX + "NiftySmallCap50FivePercentLooser" + strDate + ".csv");
		if (!file.exists()) {
			try {
				file.getParentFile().mkdir();
				file.createNewFile();
			} catch (IOException e) {
				LOG.error("Error in create file for NiftySmallCap50FivePercentLooser : " + e.getMessage());
			}
		}
		try (Writer writer = new PrintWriter(file)) {
			StatefulBeanToCsv<DailyReportModel> beanToCsv = new StatefulBeanToCsvBuilder<DailyReportModel>(writer)
			...
			LOG.error("Exception occured in daily report processing for NiftySmallCap50FivePercentLooser : "
					+ e.getMessage());
		}
