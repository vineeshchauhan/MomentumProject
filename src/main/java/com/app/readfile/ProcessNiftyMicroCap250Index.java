...
@Component
public class ProcessNiftyMicroCap250Index {

	@Value("${nifty_microcap_250}")
	String fileName;
	...
	private void saveIndexData(List<List<String>> records) {
		List<NiftyMicroCap250Model> list = new ArrayList<>();
		Iterator<List<String>> it = records.iterator();
		while (it.hasNext()) {
			List<String> row = it.next();
			if (Constants.OPEN.equals(row.get(1).strip()) || Constants.NIFTY_MICROCAP_250.equals(row.get(0))) {
				continue;
			}
			Integer symbolId = equitySymbolService.findByEquitySymbol(row.get(0)).getEquitySymbolId();
			NiftyMicroCap250Model nifty50 = niftyMicroCap250ModelService.findByEquitySymbolId(symbolId);
			if (nifty50 == null) {
				nifty50 = new NiftyMicroCap250Model();
				nifty50.setEquitySymbolId(equitySymbolService.findByEquitySymbol(row.get(0)).getEquitySymbolId());
				list.add(nifty50);
			}

			if (!list.isEmpty()) {
				niftyMicroCap250ModelService.saveAll(list);
			}
