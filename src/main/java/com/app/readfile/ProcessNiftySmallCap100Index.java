...
@Component
public class ProcessNiftySmallCap100Index {

	@Value("${nifty_smallcap_100}")
	String fileName;
	...
	private void saveIndexData(List<List<String>> records) {
		List<NiftySmallCap100Model> list = new ArrayList<>();
		Iterator<List<String>> it = records.iterator();
		while (it.hasNext()) {
			List<String> row = it.next();
			if (Constants.OPEN.equals(row.get(1).strip()) || Constants.NIFTY_SMALLCAP_100.equals(row.get(0))) {
				continue;
			}
			Integer symbolId = equitySymbolService.findByEquitySymbol(row.get(0)).getEquitySymbolId();
			NiftySmallCap100Model nifty50 = niftySmallCap100ModelService.findByEquitySymbolId(symbolId);
			if (nifty50 == null) {
				nifty50 = new NiftySmallCap100Model();
				nifty50.setEquitySymbolId(equitySymbolService.findByEquitySymbol(row.get(0)).getEquitySymbolId());
				list.add(nifty50);
			}

			if (!list.isEmpty()) {
				niftySmallCap100ModelService.saveAll(list);
			}
