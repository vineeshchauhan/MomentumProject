 ...
 */
@Component
public class PrepareNiftyMicroCap250DailyReport {
	...
	private static Logger LOG = LoggerFactory.getLogger(PrepareNiftyMicroCap250DailyReport.class);
	...
	public void createDailyReport() {
		...
		List<PriceModel> listOfPriceModelInNiftyMicroCap250 = list.stream().filter(model -> listOfEquitySymbolInNiftyMicroCap250.contains(model.getEquitySymbolId())).collect(Collectors.toList());
		// Get date
		...
		createFivePercentGainerReport(listOfPriceModelInNiftyMicroCap250, strDate, mappingStrategy);

		// prepare a list of stocks fall more than 5%
		createFivePercentLosserReport(listOfPriceModelInNiftyMicroCap250, strDate, mappingStrategy);
	}
	...
	private void createFivePercentGainerReport(List<PriceModel> list, String strDate, CustomMappingStrategy<DailyReportModel> mappingStrategy) {
		...
		File file = new File(DAILY_LOC_PREFIX + "NiftyMicroCap250FivePercentGainer" + strDate + ".csv");
		if (!file.exists()) {
			try {
	...
	private void createFivePercentLosserReport(List<PriceModel> list, String strDate, CustomMappingStrategy<DailyReportModel> mappingStrategy) {
		List<PriceModel> moreLessFivePer = list.stream()
				.filter(model -> ((model.getClosePrice() - model.getPreviousClosePrice())
						/ model.getPreviousClosePrice()) < -0.05)
				.collect(Collectors.toList());

		// prepare file for greater than five percent
		File file = new File(DAILY_LOC_PREFIX + "NiftyMicroCap250FivePercentLooser" + strDate + ".csv");
		if (!file.exists()) {
			try {
				file.getParentFile().mkdir();
				file.createNewFile();
			} catch (IOException e) {
				LOG.error("Error in create file for NiftyMicroCap250FivePercentLooser : " + e.getMessage());
			}
		}
		try (Writer writer = new PrintWriter(file)) {
			StatefulBeanToCsv<DailyReportModel> beanToCsv = new StatefulBeanToCsvBuilder<DailyReportModel>(writer)
					.withMappingStrategy(mappingStrategy).build();
			List<DailyReportModel> dailyReportModel = convertToDailyReportModel(moreLessFivePer);
		    dailyReportModel.sort(new SortDailyFivePercentLosserReport());
			beanToCsv.write(dailyReportModel);
		} catch (IOException | CsvRequiredFieldEmptyException | CsvDataTypeMismatchException e) {
			LOG.error("Exception occured in daily report processing for NiftyMicroCap250FivePercentLooser : "
					+ e.getMessage());
		}
