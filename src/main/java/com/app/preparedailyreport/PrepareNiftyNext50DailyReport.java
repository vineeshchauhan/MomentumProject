 ...
 */
@Component
public class PrepareNiftyNext50DailyReport {
	...
	private static Logger LOG = LoggerFactory.getLogger(PrepareNiftyNext50DailyReport.class);
	...
	public void createDailyReport() {
		...
		List<NiftyNext50Model> niftyNext50ModelList = niftyNext50ModelService.findAll();
		List<Integer> listOfEquitySymbolInNextNifty50 = niftyNext50ModelList.stream().map(model -> model.getEquitySymbolId()).collect(Collectors.toList());
		
		List<PriceModel> listOfPriceModelInNiftyNext50 = list.stream().filter(model -> listOfEquitySymbolInNextNifty50.contains(model.getEquitySymbolId())).collect(Collectors.toList());
		// Get date
		...
		createFivePercentGainerReport(listOfPriceModelInNiftyNext50, strDate, mappingStrategy);

		// prepare a list of stocks fall more than 5%
		createFivePercentLosserReport(listOfPriceModelInNiftyNext50, strDate, mappingStrategy);
	}
	...
	private void createFivePercentGainerReport(List<PriceModel> list, String strDate, CustomMappingStrategy<DailyReportModel> mappingStrategy) {
		...
		File file = new File(DAILY_LOC_PREFIX + "NiftyNext50FivePercentGainer" + strDate + ".csv");
		if (!file.exists()) {
			try {
				file.getParentFile().mkdir();
				file.createNewFile();
			} catch (IOException e) {
				LOG.error("Error in create file for NiftyNext50FivePercentGainer : " + e.getMessage());
			}
		}
		try (Writer writer = new PrintWriter(file)) {
			...
			LOG.error("Exception occured in daily report processing for NiftyNext50FivePercentGainer : "
					+ e.getMessage());
		}
	...
	private void createFivePercentLosserReport(List<PriceModel> list, String strDate, CustomMappingStrategy<DailyReportModel> mappingStrategy) {
		...
		// prepare file for greater than five percent
		File file = new File(DAILY_LOC_PREFIX + "NiftyNext50FivePercentLosser" + strDate + ".csv");
		if (!file.exists()) {
			try {
				file.getParentFile().mkdir();
				file.createNewFile();
			} catch (IOException e) {
				LOG.error("Error in create file for NiftyNext50FivePercentLosser : " + e.getMessage());
			}
		}
		try (Writer writer = new PrintWriter(file)) {
			StatefulBeanToCsv<DailyReportModel> beanToCsv = new StatefulBeanToCsvBuilder<DailyReportModel>(writer)
			...
			LOG.error("Exception occured in daily report processing for NiftyNext50FivePercentLosser : "
					+ e.getMessage());
		}
