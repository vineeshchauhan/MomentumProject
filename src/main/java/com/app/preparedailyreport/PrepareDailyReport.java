 ...
 */
@Component
public class PrepareDailyReport {
	...
	private static Logger LOG = LoggerFactory.getLogger(PrepareDailyReport.class);
	...
	public void createDailyReport() {
		...
		createMoreThanFivePercentReport(list, strDate, mappingStrategy);

		// prepare a list of stocks fall more than 5%
		createLessThanFivePercentReport(list, strDate, mappingStrategy);
	}

	private void createMoreThanFivePercentReport(List<PriceModel> list, String strDate, CustomMappingStrategy<DailyReportModel> mappingStrategy) {
		List<PriceModel> moreThanFivePer = list.stream()
		...
		File file = new File(DAILY_LOC_PREFIX + "GreaterThanFive" + strDate + ".csv");
		if (!file.exists()) {
			try {
				file.getParentFile().mkdir();
				file.createNewFile();
			} catch (IOException e) {
				LOG.error("Error in create file for GreaterThanFive : " + e.getMessage());
			}
		}
		try (Writer writer = new PrintWriter(file)) {
			...
			List<DailyReportModel> dailyReportModel = convertToDailyReportModel(moreThanFivePer);
			dailyReportModel.sort(new SortDailyReportForMoreThanFivePerReport());
			beanToCsv.write(dailyReportModel);
		} catch (IOException | CsvRequiredFieldEmptyException | CsvDataTypeMismatchException e) {
			LOG.error("Exception occured in daily report processing for more than five percent, lessThanFivePer : "
					+ e.getMessage());
		}
		
	}
	
	private void createLessThanFivePercentReport(List<PriceModel> list, String strDate, CustomMappingStrategy<DailyReportModel> mappingStrategy) {
		...
		// prepare file for greater than five percent
		File file = new File(DAILY_LOC_PREFIX + "LessThanFive" + strDate + ".csv");
		if (!file.exists()) {
			try {
				file.getParentFile().mkdir();
				file.createNewFile();
			} catch (IOException e) {
				LOG.error("Error in create file for LessThanFile : " + e.getMessage());
			}
		}
		try (Writer writer = new PrintWriter(file)) {
			StatefulBeanToCsv<DailyReportModel> beanToCsv = new StatefulBeanToCsvBuilder<DailyReportModel>(writer)
		    ...
		    dailyReportModel.sort(new SortDailyReportForLessThanFivePerReport());
			beanToCsv.write(dailyReportModel);
		} catch (IOException | CsvRequiredFieldEmptyException | CsvDataTypeMismatchException e) {
			LOG.error("Exception occured in daily report processing for less than five percent, lessThanFivePer : "
					+ e.getMessage());
		}
	...
	class SortDailyReportForLessThanFivePerReport implements Comparator<DailyReportModel> {
