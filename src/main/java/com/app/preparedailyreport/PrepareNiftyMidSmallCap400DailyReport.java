 ...
 */
@Component
public class PrepareNiftyMidSmallCap400DailyReport {
	...
	@Autowired
	NiftyMidSmallCap400ModelService niftyMidSmall400ModelService;

	private static Logger LOG = LoggerFactory.getLogger(PrepareNiftyMidSmallCap400DailyReport.class);
	...
	public void createDailyReport() {
		...
		List<NiftyMidSmallCap400Model> nifty400ModelList = niftyMidSmall400ModelService.findAll();
		List<Integer> listOfEquitySymbolInNifty400 = nifty400ModelList.stream().map(model -> model.getEquitySymbolId()).collect(Collectors.toList());
		
		List<PriceModel> listOfPriceModelInNifty400 = list.stream().filter(model -> listOfEquitySymbolInNifty400.contains(model.getEquitySymbolId())).collect(Collectors.toList());
		// Get date
		...
		createFivePercentGainerReport(listOfPriceModelInNifty400, strDate, mappingStrategy);

		// prepare a list of stocks fall more than 5%
		createFivePercentLosserReport(listOfPriceModelInNifty400, strDate, mappingStrategy);
	}
	...
	private void createFivePercentGainerReport(List<PriceModel> list, String strDate, CustomMappingStrategy<DailyReportModel> mappingStrategy) {
		...
		File file = new File(DAILY_LOC_PREFIX + "Nifty400FivePercentGainer" + strDate + ".csv");
		if (!file.exists()) {
			try {
				file.getParentFile().mkdir();
				file.createNewFile();
			} catch (IOException e) {
				LOG.error("Error in create file for Nifty400FivePercentGainer : " + e.getMessage());
			}
		}
		try (Writer writer = new PrintWriter(file)) {
			...
			LOG.error("Exception occured in daily report processing for, Nifty400FivePercentGainer : "
					+ e.getMessage());
		}
	...
	private void createFivePercentLosserReport(List<PriceModel> list, String strDate, CustomMappingStrategy<DailyReportModel> mappingStrategy) {
		...
		// prepare file for greater than five percent
		File file = new File(DAILY_LOC_PREFIX + "Nifty400FivePercentLooser" + strDate + ".csv");
		if (!file.exists()) {
			try {
				file.getParentFile().mkdir();
				file.createNewFile();
			} catch (IOException e) {
				LOG.error("Error in create file for Nifty400FivePercentLooser : " + e.getMessage());
			}
		}
		try (Writer writer = new PrintWriter(file)) {
			StatefulBeanToCsv<DailyReportModel> beanToCsv = new StatefulBeanToCsvBuilder<DailyReportModel>(writer)
			...
			LOG.error("Exception occured in daily report processing for Nifty400FivePercentLooser : "
					+ e.getMessage());
		}
