...
import com.app.service.impl.NiftySmallCap100ModelService;
 ...
 */
@Component
public class PrepareNiftySmallCap100DailyReport {
	...
	@Autowired
	NiftySmallCap100ModelService niftySmallCap100ModelService;

	private static Logger LOG = LoggerFactory.getLogger(PrepareNiftySmallCap100DailyReport.class);
	...
	public void createDailyReport() {
		...
		List<NiftySmallCap100Model> niftySmallCap100ModelList = niftySmallCap100ModelService.findAll();
		List<Integer> listOfEquitySymbolInNiftySmallCap100 = niftySmallCap100ModelList.stream().map(model -> model.getEquitySymbolId()).collect(Collectors.toList());
		
		List<PriceModel> listOfPriceModelInNiftySmallCap100 = list.stream().filter(model -> listOfEquitySymbolInNiftySmallCap100.contains(model.getEquitySymbolId())).collect(Collectors.toList());
		// Get date
		...
		createFivePercentGainerReport(listOfPriceModelInNiftySmallCap100, strDate, mappingStrategy);

		// prepare a list of stocks fall more than 5%
		createFivePercentLosserReport(listOfPriceModelInNiftySmallCap100, strDate, mappingStrategy);
	}
	...
	private void createFivePercentGainerReport(List<PriceModel> list, String strDate, CustomMappingStrategy<DailyReportModel> mappingStrategy) {
		...
		File file = new File(DAILY_LOC_PREFIX + "NiftySmallCap100FivePercentGainer" + strDate + ".csv");
		if (!file.exists()) {
			try {
				file.getParentFile().mkdir();
				file.createNewFile();
			} catch (IOException e) {
				LOG.error("Error in create file for NiftySmallCap100FivePercentGainer : " + e.getMessage());
			}
		}
		try (Writer writer = new PrintWriter(file)) {
			...
			LOG.error("Exception occured in daily report processing for NiftySmallCap100FivePercentGainer : "
					+ e.getMessage());
		}
	...
	private void createFivePercentLosserReport(List<PriceModel> list, String strDate, CustomMappingStrategy<DailyReportModel> mappingStrategy) {
		...
		// prepare file for greater than five percent
		File file = new File(DAILY_LOC_PREFIX + "NiftySmallCap100FivePercentLooser" + strDate + ".csv");
		if (!file.exists()) {
			try {
				file.getParentFile().mkdir();
				file.createNewFile();
			} catch (IOException e) {
				LOG.error("Error in create file for NiftySmallCap100FivePercentLooser : " + e.getMessage());
			}
		}
		try (Writer writer = new PrintWriter(file)) {
			StatefulBeanToCsv<DailyReportModel> beanToCsv = new StatefulBeanToCsvBuilder<DailyReportModel>(writer)
			...
			LOG.error("Exception occured in daily report processing for NiftySmallCap100FivePercentLooser : "
					+ e.getMessage());
		}
