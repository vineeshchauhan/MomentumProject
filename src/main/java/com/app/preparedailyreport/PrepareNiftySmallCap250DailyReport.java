 ...
 */
@Component
public class PrepareNiftySmallCap250DailyReport {
	...
	private static Logger LOG = LoggerFactory.getLogger(PrepareNiftySmallCap250DailyReport.class);
	...
	public void createDailyReport() {
		...
		List<NiftySmallCap250Model> niftySmallCap250ModelList = niftySmallCap250ModelService.findAll();
		List<Integer> listOfEquitySymbolInNiftySmallCap250 = niftySmallCap250ModelList.stream().map(model -> model.getEquitySymbolId()).collect(Collectors.toList());
		
		List<PriceModel> listOfPriceModelInNiftySmallCap250 = list.stream().filter(model -> listOfEquitySymbolInNiftySmallCap250.contains(model.getEquitySymbolId())).collect(Collectors.toList());
		// Get date
		...
		createFivePercentGainerReport(listOfPriceModelInNiftySmallCap250, strDate, mappingStrategy);

		// prepare a list of stocks fall more than 5%
		createFivePercentLosserReport(listOfPriceModelInNiftySmallCap250, strDate, mappingStrategy);
	}
	...
	private void createFivePercentGainerReport(List<PriceModel> list, String strDate, CustomMappingStrategy<DailyReportModel> mappingStrategy) {
		...
		File file = new File(DAILY_LOC_PREFIX + "NiftySmallCap250FivePercentGainer" + strDate + ".csv");
		if (!file.exists()) {
			try {
	...
	private void createFivePercentLosserReport(List<PriceModel> list, String strDate, CustomMappingStrategy<DailyReportModel> mappingStrategy) {
		...
		// prepare file for greater than five percent
		File file = new File(DAILY_LOC_PREFIX + "NiftySmallCap250FivePercentLooser" + strDate + ".csv");
		if (!file.exists()) {
			try {
				file.getParentFile().mkdir();
				file.createNewFile();
			} catch (IOException e) {
				LOG.error("Error in create file for NiftySmallCap250FivePercentLooser : " + e.getMessage());
			}
		}
		try (Writer writer = new PrintWriter(file)) {
			StatefulBeanToCsv<DailyReportModel> beanToCsv = new StatefulBeanToCsvBuilder<DailyReportModel>(writer)
			...
			LOG.error("Exception occured in daily report processing for NiftySmallCap250FivePercentLooser : "
					+ e.getMessage());
		}
